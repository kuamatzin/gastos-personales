...
class OCRService
{
    ...
    public function extractTextFromImage(string $imagePath): array
    {
        ...
        try {
            ...
            foreach ($annotations as $index => $annotation) {
                if ($index === 0) continue; // Skip the full text annotation
    ...
    private function extractPaymentMethod(string $line): ?string
    {
        ...
        if (strpos($line, 'visa') !== false) return 'visa';
        if (strpos($line, 'mastercard') !== false) return 'mastercard';
        if (strpos($line, 'amex') !== false || strpos($line, 'american express') !== false) return 'amex';
        if (strpos($line, 'debit') !== false || strpos($line, 'débito') !== false) return 'debit';
        if (strpos($line, 'credit') !== false || strpos($line, 'crédito') !== false) return 'credit';
        if (strpos($line, 'cash') !== false || strpos($line, 'efectivo') !== false) return 'cash';
    ...
    private function isPotentialMerchant(string $line): bool
    {
        ...
        if (preg_match('/\$\s*[\d.,]+/', $line)) return false;
        if (preg_match('/\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}/', $line)) return false;
