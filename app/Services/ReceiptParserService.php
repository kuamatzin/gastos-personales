...
class ReceiptParserService
{
    ...
    private function calculateConfidence(array $lines): float
    {
        ...
        if ($this->extractMerchant($lines)) $score++;
        if ($this->extractTotal($lines)) $score++;
        if ($this->extractDate($lines)) $score++;
        if ($this->extractPaymentMethod($lines)) $score++;
        if (count($this->extractLineItems($lines)) > 0) $score++;
        if ($this->extractReferenceNumber($lines)) $score++;
